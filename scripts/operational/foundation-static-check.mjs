...
const modules = seed.track.modules,
  topics = modules.flatMap((m) => m.topics),
  contents = topics.flatMap((t) => t.contents),
  activities = topics.flatMap((t) => t.activities),
  materials = topics.flatMap((t) => t.materials),
  projects = seed.suggested_projects;
const counts = {
...
const runbooks = [
  ...
  "ROLLBACK_FORWARD_FIX.md",
  "BACKUP.md",
  "RESTORE_DR.md",
  "SEED_IMPORT.md",
  "SEED_CONFLICT_TOMBSTONE.md",
  "STORAGE_RECONCILIATION.md",
  "COMPROMISED_SECRET.md",
  "AUTH_INCIDENT.md",
  "PRODUCTION_SMOKE.md",
];
...
for (const f of files) {
  if (!/\.(ts|tsx|js|mjs|json|yml|yaml|md|sql|toml|sh|example)$/.test(f)) continue;
  ...
  JSON.stringify(
    {
      ...
      pass,
      fail,
      seedSha256: crypto.createHash("sha256").update(fs.readFileSync(seedPath)).digest("hex"),
    },
    null,
    2,
  ),
