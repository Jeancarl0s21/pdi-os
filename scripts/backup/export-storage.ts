...
if (!url || !key || !outDir)
  throw new Error(
    "Usage: export-storage.ts <output-dir> with NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY",
  );
...
async function walk(bucket: string, prefix = ""): Promise<string[]> {
  ...
  while (true) {
    ...
    for (const x of data) {
      const full = prefix ? `${prefix}/${x.name}` : x.name;
      if (x.id) out.push(full);
...
for (const bucket of buckets) {
  for (const object of await walk(bucket)) {
    const { data, error } = await sb.storage.from(bucket).download(object);
    if (error) throw error;
    const dest = path.join(outDir, bucket, object);
