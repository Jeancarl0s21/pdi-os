export const uploadPolicy = {
  ...
  evidence: { maxBytes: 5 * 1024 * 1024, mime: ['application/pdf','image/jpeg','image/png','image/webp','text/plain','text/csv','application/json','application/x-ipynb+json'] },
} as const;
...
export function validateDeclaredUpload(kind: UploadKind, size: number, mime: string) {
  ...
  if (!Number.isInteger(size) || size <= 0 || size > policy.maxBytes) return { ok:false as const, reason:'size' };
  if (!(policy.mime as readonly string[]).includes(mime)) return { ok:false as const, reason:'mime' };
...
export function validateMagicBytes(mime:string, bytes:Uint8Array) {
  ...
  if (mime==='image/png') return bytes.slice(0,8).every((b,i)=>b===[137,80,78,71,13,10,26,10][i]);
  ...
  if (mime==='application/json'||mime==='application/x-ipynb+json') { try { JSON.parse(new TextDecoder().decode(bytes)); return true; } catch { return false; } }
